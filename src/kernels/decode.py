...
from kernels.transformer import CausalMHA, Impl, OPS
...
class CausalMHAInfer(CausalMHA):
    ...
    def __init__(
        ...
        self.rotary_embedding = OPS[impl]["rotary_embedding"]
    ...
    def decode(self, x: Float[torch.Tensor, "batch 1 d_model"]) -> Float[torch.Tensor, "batch 1 d_model"]:
        ...
        cos = torch.cos(freqs)
        sin = torch.sin(freqs)
        tok_q = self.rotary_embedding(tok_q, cos, sin)
        tok_k = self.rotary_embedding(tok_k, cos, sin)
